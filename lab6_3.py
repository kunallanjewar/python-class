import time 
import datetime  
t = datetime.datetime.now() # get the current date 
...
# int() does type cast 
msg = "Today is " + week[w] + " the " + day[int(d)-1] + " day of " + month[int(m)-1] + ", " + Y + "."     
print(msg)  
### SuperLotto 
z = []  
...
n = int(input("How many drawing(s)? "))  
j = 0  
while (j < n):  
 s = ""    
 shuffle(z)    
 ...
 for i in range(0, 5):   
  s = s + str(z[i]) + " "
 s = s + "mega: " + str(random.randint(0, 27))    
 print(s)   
 j = j + 1